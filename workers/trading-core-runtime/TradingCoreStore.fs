...
type TradingCoreStore(config: ServiceConfig, json: JsonSerializerOptions,
                      secrets: SecretStore, clock: TimeProvider) =
    do Database.initialize config.DatabasePath config.InitialMode (clock.GetUtcNow().UtcDateTime)
    ...
    member internal _.UtcNow = clock.GetUtcNow().UtcDateTime
