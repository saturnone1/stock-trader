...
if [[ "$deploy_scope" != "all" && "$deploy_scope" != "api" && "$deploy_scope" != "desktop" ]]; then
  echo "STOCKTRADER_DEPLOY_SCOPE must be all, api, or desktop." >&2
  exit 1
fi
...
if [[ "$deploy_scope" != "desktop" ]] && ! sudo k3s kubectl -n stocktrader get secret stocktrader-alpaca >/dev/null 2>&1; then
...
if [[ "$deploy_scope" != "desktop" ]] && sudo k3s kubectl -n stocktrader get deployment stocktrader-api >/dev/null 2>&1; then
