...
valid_days="${STOCKTRADER_ML_TRAINING_TLS_DAYS:-90}"
generation="${STOCKTRADER_ML_TRAINING_TLS_GENERATION:-$(date -u +%Y%m%d%H%M%S)}"
if [[ ! "$valid_days" =~ ^[0-9]+$ ]] || (( valid_days < 7 || valid_days > 397 )); then
  echo "STOCKTRADER_ML_TRAINING_TLS_DAYS must be between 7 and 397." >&2; exit 1
fi
if [[ ! "$generation" =~ ^[a-z0-9][a-z0-9-]{0,13}$ ]]; then
  echo "STOCKTRADER_ML_TRAINING_TLS_GENERATION must be 1-14 lowercase letters, digits, or hyphens." >&2; exit 1
fi
server_secret="stocktrader-ml-training-server-tls-$generation"
client_secret="stocktrader-ml-training-client-tls-$generation"
sudo k3s kubectl apply -f k8s/namespace.yaml
tls_dir="$(mktemp -d /tmp/stocktrader-ml-training-tls.XXXXXX)"
cleanup() {
...
openssl req -x509 -new -sha256 -days 397 -key "$tls_dir/ca.key" \
  -out "$tls_dir/ca.crt" -subj "/CN=StockTrader ML Training Internal CA" >/dev/null 2>&1
...
openssl req -new -sha256 -key "$tls_dir/server.key" -out "$tls_dir/server.csr" \
  -subj "/CN=stocktrader-ml-training" >/dev/null 2>&1
printf '%s\n' 'basicConstraints=critical,CA:FALSE' \
  'keyUsage=critical,digitalSignature,keyEncipherment' 'extendedKeyUsage=serverAuth' \
  'subjectAltName=DNS:stocktrader-ml-training,DNS:stocktrader-ml-training.stocktrader,DNS:stocktrader-ml-training.stocktrader.svc' \
  > "$tls_dir/server.ext"
...
sudo k3s kubectl -n "$namespace" create secret generic "$client_secret" \
  ...
  --from-file=ca.crt="$tls_dir/ca.crt" --dry-run=client -o yaml | sudo k3s kubectl apply -f -
sudo k3s kubectl -n "$namespace" create configmap stocktrader-ml-training-tls-active \
  --from-literal=generation="$generation" --dry-run=client -o yaml | sudo k3s kubectl apply -f -
echo "ML Training TLS generation $generation created for $namespace; redeploy service and API together."
