<script>
  ...
  const timingStructureOptions = [
    { id: 'market', label: '시장 타이밍' },
    { id: 'market-stock', label: '시장 + 종목 타이밍' }
  ]

  const factorExperimentPresets = [
    {
      id: 'value-pe',
      label: '저PER 흑자',
      note: 'PER 15 이하 + 흑자',
      params: { peRatioMax: '15', positiveEarningsOnly: true, sortBy: 'peAsc' }
    },
    {
      id: 'value-pb',
      label: '저PBR 자산주',
      note: 'PBR 1.5 이하 + 흑자',
      params: { pbRatioMax: '1.5', positiveEarningsOnly: true, sortBy: 'pbAsc' }
    },
    {
      id: 'quality-roe',
      label: '퀄리티 ROE',
      note: 'ROE 12% 이상 + 영업이익률 8% 이상',
      params: { roePercentMin: '12', operatingMarginMin: '8', positiveEarningsOnly: true, sortBy: 'roeDesc' }
    },
    {
      id: 'revenue-growth',
      label: '매출 성장',
      note: '매출 성장 10% 이상',
      params: { revenueGrowthMin: '0.1', positiveEarningsOnly: true, sortBy: 'revenueGrowthDesc' }
    },
    {
      id: 'earnings-growth',
      label: '이익 성장',
      note: '순이익 성장 15% 이상',
      params: { netIncomeGrowthMin: '0.15', positiveEarningsOnly: true, sortBy: 'netIncomeGrowthDesc' }
    },
    {
      id: 'turnaround-growth',
      label: '턴어라운드 성장',
      note: '턴어라운드 + 매출/순이익 성장',
      params: { turnaroundOnly: true, revenueGrowthMin: '0.1', netIncomeGrowthMin: '0.1', positiveEarningsOnly: true, sortBy: 'netIncomeGrowthDesc' }
    }
  ]

  const factorRankingOptions = [
  ...
  function intersectSymbols(baseSymbols, filterSymbols) {
  ...
  function whipsawThresholdBars(timeframe) {
    switch (timeframe) {
      case 'OneMinute': return 30
      case 'FiveMinute': return 12
      case 'FifteenMinute': return 8
  ...
  function estimateHoldingBars(trade, timeframe) {
    ...
    return Math.max(1, Math.round(diffMinutes / timeframeBarMinutes(timeframe)))
  }

  function getWhipsawStats(sourceResult) {
    const trades = sourceResult?.trades ?? []
    if (!trades.length) {
      return { count: 0, rate: 0, thresholdBars: whipsawThresholdBars(sourceResult?.usedTimeFrame) }
    }

    const thresholdBars = whipsawThresholdBars(sourceResult?.usedTimeFrame)
    const count = trades.filter((trade) => Number(trade.pnlPercent ?? trade.PnLPercent ?? 0) < 0 && estimateHoldingBars(trade, sourceResult?.usedTimeFrame) <= thresholdBars).length
    return {
      count,
      rate: count / trades.length,
      thresholdBars
    }
  }

  function getEquityCurveVolatility(sourceResult) {
    const curve = sourceResult?.equityCurve ?? []
    if (curve.length < 3) return null

    const returns = []
    for (let index = 1; index < curve.length; index += 1) {
      const prev = Number(curve[index - 1]?.equity ?? curve[index - 1]?.Equity ?? 0)
      const next = Number(curve[index]?.equity ?? curve[index]?.Equity ?? 0)
  ...
  <div class="space-y-8 p-8">
    ...
    {#if result}
      <section class="space-y-4">
        ...
        {#if getTimingReport(getActiveComparisonEntry())}
          {@const timingReport = getTimingReport(getActiveComparisonEntry())}
